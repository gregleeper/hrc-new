...
import { faChild, faHandsHelping, faPaintBrush, faRunning } from '@fortawesome/free-solid-svg-icons';

export default function FacilityItem({ activity }) {
    function ShowSportIcon() {
        if (activity.activityName.toLowerCase().includes('cheer')) {
            return <FontAwesomeIcon icon={faChild} className="text-red-600 text-3xl" />;
        }
