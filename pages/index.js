...
export default function Home({ data }) {
    ...
    const sorted = data?.data?.listEvents?.items?.sort((a, b) => new Date(a.dateAndTime) - new Date(b.dateAndTime));

    const sortedAndFiltered = sorted?.filter((e) => moment(e.dateAndTime).add(8, 'hours') >= moment(today));
    ...
    return (
        <div className="relative bg-white overflow-hidden">
            ...
            <div className="lg:absolute lg:inset-y-0 lg:right-0 lg:w-1/2">
                <Image src={mainOffice} height="800" width="1000" alt="tball game" objectFit="initial" />
            </div>

            <div>
                <div className="relative bg-white overflow-hidden">
                    <div className="pb-5 border-b border-gray-200 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                        <h3 className=" leading-6 font-bold text-gray-900 py-3 text-xl">Upcoming Events</h3>
                    </div>
